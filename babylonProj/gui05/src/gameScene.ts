...
function createGround(scene: Scene) {
  ...
  const groundTexture = new Texture("./assets/textures/floor.png");
  groundTexture.uScale  = 8.0; //Repeat 5 times on the Vertical Axes
  groundTexture.vScale  = 8.0; //Repeat 5 times on the Horizontal Axes
 ...
 // groundMaterial.diffuseTexture = new Texture("./assets/textures/wood.jpg");
  ...
  let ground = MeshBuilder.CreateGround(
    "ground",
    { width: 60, height: 60, subdivisions: 8 },
    scene
  );
...
function createBox1(scene: Scene) {
  ...
  var texture = new StandardMaterial("crate", scene);
